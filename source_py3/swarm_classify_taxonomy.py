...
def get_taxonomy(fasta_file, ggsearch_file, database_file, cpus):
    ...
    if fasta_file:
        if os.path.exists(ggsearch_file):
            ...
            print("[swarm_classify_taxonomy] running ggsearch", file=sys.stderr)
            ...
            rc = os.system(cmd)
            if rc != 0:
                print("[swarm_classify_taxonomy] ERROR: ggsearch", file=sys.stderr)
                sys.exit(2)
...
def test_all():
    print("[swarm_classify_taxonomy] test_all: passed", file=sys.stderr)
